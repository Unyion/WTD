...
function setupEventListeners() {
    ...
    // Weather display click for manual refresh
    weatherDisplay.addEventListener('click', () => {
        ...
        if (weatherText.includes('Location not found') || weatherText.includes('Unable to get location')) {
            ...
            if (canRefreshWeather()) {
                ...
                showNotification(`Please wait ${remaining} seconds before refreshing again`, 'warning');
            }
        }
    });
    
    // Weather display hover for tooltip
    weatherDisplay.addEventListener('mouseenter', (e) => {
        if (lastWeatherUpdate) {
            const minutesAgo = Math.floor((Date.now() - lastWeatherUpdate) / (1000 * 60));
            const timeText = minutesAgo === 0 ? 'less than a minute ago' : 
                           minutesAgo === 1 ? '1 minute ago' : 
                           `${minutesAgo} minutes ago`;
            
            const tooltipText = `Last updated ${timeText}. Click to refresh.`;
            showTooltip(e, tooltipText);
        } else {
            showTooltip(e, 'Click to refresh weather');
        }
    });
...
// Weather rate limiting functions
function canRefreshWeather() {
    const now = Date.now();
    const oneMinute = 60 * 1000;
    
    // Reset counter if a minute has passed
    if (now - weatherUpdateResetTime >= oneMinute) {
        weatherUpdateCount = 0;
        weatherUpdateResetTime = now;
    }
    
    return weatherUpdateCount < 3;
}

function getRemainingCooldownTime() {
    const now = Date.now();
    const oneMinute = 60 * 1000;
    const timeSinceReset = now - weatherUpdateResetTime;
    const timeUntilReset = oneMinute - timeSinceReset;
    
    return Math.ceil(timeUntilReset / 1000);
}

function incrementWeatherUpdateCount() {
    const now = Date.now();
    const oneMinute = 60 * 1000;
